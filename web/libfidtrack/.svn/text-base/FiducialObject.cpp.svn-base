...
void FiducialObject::update (float x, float y, float a, float root, float leaf) {
	...
	saveLastFrame();
	//std::cout << "motion accel  " << current.motion_accel << "\nmotion speed  " << current.motion_speed << "\nangle speed  " << current.rotation_speed << "\ntime  " << current.time << std::endl;
...
bool FiducialObject::removalFilter() {
	...
	if (!updated) {
		...
		if (lost_frames>frame_threshold) {
			alive = false;
			current.xpos = current.ypos = current.raw_xpos = current.raw_ypos = 9999.0f;
			current.angle = current.raw_angle = 9999.0f;
			current.rotation_speed = current.rotation_accel = 9999.0f;
			current.motion_speed = current.motion_accel = 9999.0f;
			current.motion_speed_x = current.motion_speed_y = 9999.0f; 
			current.time = 0;
			lost_frames = 0;
			return true;
		} else {
