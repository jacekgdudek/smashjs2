...
TutorialScene.prototype = new Scene(tutorialSceneStructure);

/*
var currentHint, hints, delay, startRotation, volumeTarget, wentPastTarget; // for relative roation conditions
...
TutorialScene.prototype.init = function() {
	...
	stethoscope = this.structure.visuals[3];			
...
TutorialScene.prototype.update = function() {
	...
	if(this.hintDelayCounter > this.hintDelay)
	{
		...
		if (this.currentHint >= this.hints.length) {
			...
			addEventEx(this.structure.nextStageEvent);
			return;
			// Aww yeah!
		}
		...
		if(typeof this.hints[this.currentHint].message !== 'undefined')
		{
			if(this.hints[this.currentHint].message !=  this.structure.messages[0].text.text)
			{
				...
				this.structure.messages[0].bg.visible = true;
				this.structure.messages[0].text.visible = true;
				this.stage.addChild(this.structure.messages[0].bg)
			}
	...
	{
		this.hintDelayCounter++;
	}
};

TutorialScene.prototype.finalize = function() {
...
TutorialScene.prototype.handleKeyDown = function(evt) {
	Scene.prototype.handleKeyDown.call(this);
