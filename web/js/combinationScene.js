var combinationScene = (function() {
	...
	return {
		init: function() {
			...
			rotatingDial.x = 900+139;
			rotatingDial.y = 200+139;
			rotatingDial.regX += 139;
			rotatingDial.regY += 139;

			//rotatingIndicator = new createjs.Bitmap("../media/visuals/safe/indicator.png");
			rotatingIndicator = this.scene.visuals[1];			
			rotatingIndicator.x = 900+139;
			rotatingIndicator.y = 200+139;
			rotatingIndicator.regX += 139;
			rotatingIndicator.regY += 139;
		...
		update: function() {
			...
			rotatingDial.rotation = this.input.rotation;
			
			if (this.spin < spinDuration) {
				// Otherwise use it to show progress (i.e. the value of targetPointer)
				rotatingIndicator.rotation = 90*this.targetPointer+(this.spin*spinSpeed);
			}
