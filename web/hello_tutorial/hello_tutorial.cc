...
class HelloTutorialInstance : public pp::Instance {
 ...
 public:
    ...
    if(var_message.is_array_buffer())
    {
        ...
        //data[6] = 'c';
        std::stringstream ss;// << " y: " << fid0_y << " ~~  Fid2 x:" << fid2_x << " y: " << fid2_y;
        std::string str = "";
        //ss >> str;
        //PostMessage(str);
        ...
        for (frame = processorList.begin(); frame!=processorList.end(); frame++) {
            ...
            for (std::list<FiducialObject>::iterator fiducial = fiducialList.begin(); fiducial!=fiducialList.end(); ++fiducial) {
                ...
                if (fiducial->fiducial_id == 0){
                    ...
                    //cv::putText(mRGBcam, "R", cv::Point(640-(fo->getX()-10)-10,fo->getY()+20), 2, 1, cv::Scalar(57,181,74));
                }
                    ...
                    fid2_found = true;
                    //cv::circle(mRGBcam, cv::Point(640-(fo->getX()-5),fo->getY()+10), 50, cv::Scalar(0,201,238),-1);
                    //cv::putText(mRGBcam, "L", cv::Point(640-(fo->getX()-10)-10,fo->getY()+20), 2, 1, cv::Scalar(57,181,74));
                }
            }
            ...
            //std::stringstream ss;
            if(fid0_found)ss << 0 << "," << fid0_x << "," << fid0_y;// << " y: " << fid0_y << " ~~  Fid2 x:" << fid2_x << " y: " << fid2_y;
            
            if(fid0_found && fid2_found) ss << ",";
            
            if(fid2_found)ss << 2 << "," << fid2_x << "," << fid2_y;
            //std::string str = "";
            ss >> str;
